...
public class AccessHost_DeliveryStatus extends com.avaya.sce.runtime.Data {
	...
	public void requestBegin(SCESession mySession) {
		...
		try {
			...
			/*strDeveliveryStatusDateFormat = appProperties.getIVRProp(AppConstant.DeveliveryStatusDateFormat);
			...
			{
				reqBillDeliveryStatus.setMDN(mySession.getVariableField(IProjectVariables.APP_VARIABLES, IProjectVariables.APP_VARIABLES_FIELD_MOBILE_NO).getStringValue());
			}
			...
			if((resBillDeliveryStatus.getResponseCode()).equals(AppConstant.zero)){
				...
				strSMSFrame = appProperties.getIVRProp(AppConstant.DELIVERY_STATUS_MODE_Success);
				smsHashTable = setPushSMS.generatePushSMSDataMap(strSMSFrame, arrSMSData);
				
				if(strMode != null){
					if((strMode.equalsIgnoreCase(AppConstant.POST))||(strMode.equalsIgnoreCase(AppConstant.COURIER))){
					...
					if(dateBillGenDate.compareTo(callDate)<0){
						mySession.getVariableField(IProjectVariables.BILL_DELIVERY_STATUS, IProjectVariables.BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(AppConstant.Yes);
					}else{
						mySession.getVariableField(IProjectVariables.BILL_DELIVERY_STATUS, IProjectVariables.BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(AppConstant.No);
					}
				}else{
					mySession.getVariableField(IProjectVariables.BILL_DELIVERY_STATUS, IProjectVariables.BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(AppConstant.No);
				}
				...
				strSMSFrame = appProperties.getIVRProp(AppConstant.DELIVERY_STATUS_MODE_Failure);
				smsHashTable = setPushSMS.generatePushSMSDataMap(strSMSFrame, arrSMSData);
	...
	public com.avaya.sce.runtime.Next getNext(com.avaya.sce.runtimecommon.SCESession mySession) {
		com.avaya.sce.runtime.Next next = new com.avaya.sce.runtime.Next("PostpaidAccountInformation-CheckBillDeliveryStatus", "Default");
	...
	public boolean executeDataActions(com.avaya.sce.runtimecommon.SCESession mySession) throws Exception {
		...
		if(((com.avaya.sce.runtime.Condition)new com.avaya.sce.runtime.Condition("condition1", "Flag:isLinkDown", com.avaya.sce.runtime.Expression.STRING_EQUAL_IGNORE, "Constants:yes", true).setDebugId(4976)).evaluate(mySession)) {
			actions = new java.util.ArrayList(2);
			actions.add(new com.avaya.sce.runtime.varoperations.Assign("appVariables:transferReason", "TR_0037", true).setDebugId(5133));
			actions.add(new com.avaya.sce.runtime.Next("PostpaidAccountInformation-Linkdown", "LinkDown").setDebugId(4977));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;
