...
public class TransValidateAccountNumber implements ITransaction{

	ReqValidateAccountNumber reqValidateAccountNumber = null;
	ResValidateAccountNumber resValidateAccountNumber = new ResValidateAccountNumber();
	...
	public ResValidateAccountNumber getHostInterface(IVRDataBean ivrDataBean, ReqValidateAccountNumber request){
		...
		try{
			...
			host.setHOSTID(HostTransactionID.TRANS_VALIDATE_ACCOUNT_NUMBER);
			host.setHOSTMETHOD(HostTransactionID.TRANS_VALIDATE_ACCOUNT_NUMBER);
			
			reqValidateAccountNumber = request;
			uniqueId = uniqueId+HostConstants.DOT+ivrDataBean.getSessionID();
			logger.debug(uniqueId + "|" + "TransValidateAccountNumber getHostInterface method start");
			processor();
			logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.debug(uniqueId + "|" + "TransValidateAccountNumber getHostInterface method end");
	...
	public void processor() {
		...
		try {
			...
			accountNo = reqValidateAccountNumber.getAccountNo();
			...
			strURL = hostProperties.getIVRProp(HostConstants.ENDPOINT_URL_VALIDATE_ACCOUNT_NUMBER);
			...
			qs = new QueryString(HostConstants.FIELD1, accountNo);
			...
			if(responseMessage!= null && !responseMessage.equals(HostConstants.EMPTY) && responseMessage.length() != 0)
			{
				...
				if(strResMessage.split(HostConstants.COMMA_SEPERATOR).length > 1)
				{
					...
					validationFlag = resFields[1];
					
					if(validationFlag.equalsIgnoreCase(HostConstants.STR_1))
					{
						if(resFields.length>2) consumerName = resFields[2];
						
						resValidateAccountNumber.setValidAccount(true);
						resValidateAccountNumber.setConsumerName(consumerName);
						
					}
					else
					{
						resValidateAccountNumber.setValidAccount(false);
					}
					
					
					logger.debug(uniqueId + "|" + "Validation Flag : " +validationFlag);
					logger.debug(uniqueId + "|" + "Consumer Name : " +consumerName);
			...
			resValidateAccountNumber.setResponseCode(responseCode);
