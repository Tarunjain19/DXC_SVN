...
public class TransAccountBalance implements ITransaction{
	...
	public ResAccountBalance getHostInterface(IVRDataBean ivrDataBean, ReqAccountBalance request) {
		...
		try{
			...
			logger.debug(uniqueId + "|" + "TransAccountBalance getHostInterface method start");
			wsdlProcessor();
	...
	public void wsdlProcessor() {
		...
		try {
			...
			logger.debug(uniqueId + "|" + "Before calling accountBalance Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					logger.debug(uniqueId + "|" + "Account Balance : " +accBal);
					logger.debug(uniqueId + "|" + "Account Balance Expiry Date : " +accBalExpDate);
