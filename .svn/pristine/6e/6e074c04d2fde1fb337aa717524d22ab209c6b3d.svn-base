...
public class TransLastRechargeDetails  implements ITransaction{
	...
	private String errorCode = null,Lrdamt = null,Lrdttime =null;
	...
	public ResLastRechargeDetails getHostInterface(IVRDataBean ivrDataBean, ReqLastRechargeDetails request) {
		...
		try{
			...
			logger.debug(uniqueId + "|" + "TransLastRechargeDetails getHostInterface method start");
			wsdlProcessor();
			logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.debug(uniqueId + "|" + "TransLastRechargeDetails getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			logger.debug(uniqueId + "|" + "Before calling lastRechargeDetails Service");
			...
			request = null;
			logger.debug(uniqueId + "|" + "After calling lastRechargeDetails Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					logger.debug(uniqueId + "|" + "Total Recharges : " +lastThreeRechargeTokens.size());
					...
					if(lastThreeRechargeTokens.size() != 0)
					{
						for(Token rechargeToken : lastThreeRechargeTokens)
						{
							...
							for(Param param : params)
							{
								...
								{
									Lrdamt = param.getValue();
									Lrdamt = Utilities.adjustDecimal(logger, Lrdamt);
									rechargeDetails.setAmount(Lrdamt);
								}
								...
								{
									Lrdttime = param.getValue();
									Lrdttime = Utilities.adjustDecimal(logger, Lrdttime);
									rechargeDetails.setTalktime(Lrdttime);
								}
					...
					{
						logger.debug(uniqueId + "|" + "Recharge Details : " +rechrgeDetails1.toString());
					}
