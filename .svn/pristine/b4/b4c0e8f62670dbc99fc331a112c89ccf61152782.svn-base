...
import com.servion.rcom.broadband.hostinterface.util.AESCryptoService;
...
import com.servion.rcom.broadband.hostinterface.view.ERecharge;
...
public class TransURLHitPaymentUsingCreditcard implements ITransaction{
	...
	public ResPaymentUsingCreditcard getHostInterface(IVRDataBean ivrDataBean, ReqPaymentUsingCreditcard request) {
		...
		try{
			...
			host.setHOSTID(HostTransactionID.TRANS_BILL_PAY_BY_CREDIT_CARD);
			
			reqPaymentValidation = request;
			uniqueId = uniqueId+HostConstants.DOT+ivrDataBean.getSessionID();
			logger.debug(uniqueId + "|" + "TransURLHitPaymentValidation getHostInterface method start");
			wsdlProcessor();
			logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.debug(uniqueId + "|" + "TransURLHitPaymentValidation getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			logger.debug(uniqueId + "|" + "Encrypted xml : " +xml);
			...
			responseCode = HostConstants.RESPONSE_CODE_FAILURE_PAYMENT_PROCESS;
