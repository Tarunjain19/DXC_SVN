...
public class TransCreditLimitInfo implements ITransaction{

	ReqCreditLimitInfo reqCreditLimitInfo = null;
	ResCreditLimitInfo resCreditLimitInfo = new ResCreditLimitInfo();
	...
	public ResCreditLimitInfo getHostInterface(IVRDataBean ivrDataBean, ReqCreditLimitInfo request) {
		...
		host.setHOSTID(HostTransactionID.TRANS_CREDIT_LIMIT);
		
		reqCreditLimitInfo = request;
		uniqueId = ivrDataBean.getSessionID();
		logger.debug(uniqueId + "|" + "TransCreditLimitInfo getHostInterface method start");
		wsdlProcessor();
		logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
		logger.debug(uniqueId + "|" + "TransCreditLimitInfo getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			serviceURL = appProperties.getIVRProp(HostConstants.CSS_ENDPOINT_URL_CL);
			...
			request.setRequestType(appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_CREDIT_LIMIT_INFO));
			...
			logger.debug(uniqueId + "|" + "Before calling credit limit info Service");
			
			//For Reporting
			host.setHOSTMETHOD(HostConstants.CSS_HOST_METHOD + HostConstants.PIPE + appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_CREDIT_LIMIT_INFO));
			...
			request = null;
			logger.debug(uniqueId + "|" + "After calling credit limit info Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					//Searching Credit Limit Info token through Parameter names
					Token creditLimitInfoToken = null;
					for(Token token : tokens)
					{
						...
						{
							if(param.getId().equalsIgnoreCase(HostConstants.PARAM_CL_CREDIT_LIM))
							{
								
								creditLimitInfoToken = token;
									break;
					...
					if(creditLimitInfoToken != null)
					{
						List<Param> listParam = creditLimitInfoToken.getMsgParam().getParam();
						
						for(Param param : listParam)
						{
							if(param.getId().equalsIgnoreCase(HostConstants.PARAM_CL_CREDIT_LIM))
							{
								creditLimit = param.getValue();
							}
							if(param.getId().equalsIgnoreCase(HostConstants.PARAM_CL_SYSDATE))
							{
								sysDate = param.getValue();
							}
						}
					}
					
					
					logger.debug(uniqueId + "|" + "Credit Limit Amount" +creditLimit);
					logger.debug(uniqueId + "|" + "Credit Limit  System Date : " +sysDate);
					
					resCreditLimitInfo.setCreditLimit(creditLimit);
					resCreditLimitInfo.setSysDate(sysDate);
					
					
					msgTokens = null;
					tokens = null;
					creditLimitInfoToken = null;
			...
			resCreditLimitInfo.setResponseCode(responseCode);
			
			//For Reporting
			host.setHOSTOUTPARAMS(errorCode+HostConstants.PIPE+resCreditLimitInfo.toString());
