...
public class AccessHost_DeliveryStatus extends com.avaya.sce.runtime.Data {
	...
	public void requestBegin(SCESession mySession) {
		...
		try {
			...
			reqBillDeliveryStatus.setMDN(mySession.getVariableField(IProjectVariables.APP_VARIABLES, IProjectVariables.APP_VARIABLES_FIELD_ANI).getStringValue());
			...
			mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_RESPONSE_CODE).setValue(resBillDeliveryStatus.getResponseCode());
			if((resBillDeliveryStatus.getResponseCode()).equals(Constants.zero)){
				...
				mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_BILL_DELIVERY_DATE).setValue(strBillDeliveryDate);
				mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_BILL_DELIVERY_STATUS).setValue(resBillDeliveryStatus.getBillDeliveryStatus());
				mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_BILL_GENERATION_DATE).setValue(strBillGenDate);
				mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_MODE).setValue(strMode);
				...
				info.writeln(ITraceInfo.TRACE_LEVEL_DEBUG, "--->Bill Date Diff:"+dateDiff);
				mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_BILL_GEN_DATE_DIFF).setValue(dateDiff);
				...
				if(strMode != null){
					if((strMode.contains(Constants.POST))||(strMode.contains(Constants.COURIER))){
					...
					if(dateBillGenDate.compareTo(callDate)<0){
						mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(Constants.Yes);
					}else{
						mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(Constants.No);
					}
				}else{
					mySession.getVariableField(IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS, IProjectVariables.POSTPAID__BILL_DELIVERY_STATUS_FIELD_ETA_FLAG).setValue(Constants.No);
				}
	...
	public boolean executeDataActions(com.avaya.sce.runtimecommon.SCESession mySession) throws Exception {
		...
		actions.add(new com.avaya.sce.runtime.varoperations.Assign("Postpaid_CallData:repeatTries", "0", true).setDebugId(5305));
		if(evaluateActions(actions, mySession)) {
			return true;
		}
		actions = null;

		if(((com.avaya.sce.runtime.Condition)new com.avaya.sce.runtime.Condition("condition1", "Postpaid_Flag:isLinkDown", com.avaya.sce.runtime.Expression.STRING_EQUAL_IGNORE, "Postpaid_Constants:yes", true).setDebugId(4976)).evaluate(mySession)) {
			actions = new java.util.ArrayList(1);
			actions.add(new com.avaya.sce.runtime.varoperations.Assign("Postpaid_Flag:isLinkDown", "Postpaid_Constants:no", false).setDebugId(5856));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;

			if(((com.avaya.sce.runtime.Condition)new com.avaya.sce.runtime.Condition("condition2", "Flag:CalledFromIntelligent", com.avaya.sce.runtime.Expression.STRING_EQUAL_IGNORE, "Postpaid_Constants:yes", true).setDebugId(5892)).evaluate(mySession)) {
				actions = new java.util.ArrayList(1);
				actions.add(new com.avaya.sce.runtime.Next("AccountInformation-MainMenu", "(Main Menu)").setDebugId(5893));
				if(evaluateActions(actions, mySession)) {
					return true;
				}
				actions = null;
