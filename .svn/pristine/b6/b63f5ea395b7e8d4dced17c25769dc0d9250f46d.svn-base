...
public class TransDNCregistration implements ITransaction{

	ReqDNCregistration reqDNCregistration = null;
	ResDNCregistration resDNCregistration = new ResDNCregistration();
	...
	public ResDNCregistration getHostInterface(IVRDataBean ivrDataBean , ReqDNCregistration request) {
		...
		try{
			...
			host.setHOSTID(HostTransactionID.TRANS_DNC_REGISTRATION);
			
			reqDNCregistration = request;
			uniqueId = uniqueId+HostConstants.DOT+ivrDataBean.getSessionID();
			logger.debug(uniqueId + "|" + "TransDNCregistration getHostInterface method start");
			wsdlProcessor();
			logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.debug(uniqueId + "|" + "TransDNCregistration getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			serviceURL = LoadProperty.getivrconfigProp(HostConstants.CSS_ENDPOINT_URL_DNC_ACT);
			...
			request.setRequestType(LoadProperty.getivrconfigProp(HostConstants.CSS_REQUEST_TYPE_DNC_ACTIVATION));
			...
			DNDInfo.setValue(HostConstants.PARAM_DND_ACTION_TYPE_ACTIVATION);
			...
			logger.debug(uniqueId + "|" + "Before calling DND Activation Service");
			
			//For Reporting
			host.setHOSTMETHOD(HostConstants.CSS_HOST_METHOD + HostConstants.PIPE + LoadProperty.getivrconfigProp(HostConstants.CSS_REQUEST_TYPE_DNC_ACTIVATION));
			...
			request = null;
			logger.debug(uniqueId + "|" + "After calling DND Activation Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					resDNCregistration.setMessageText(messageText);
			...
			resDNCregistration.setResponseCode(responseCode);
			
			//For Reporting
			host.setHOSTOUTPARAMS(errorCode+HostConstants.PIPE+resDNCregistration.getMessageText());
