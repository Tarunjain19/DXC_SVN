...
public class TransNonActivePackDetails implements ITransaction{
	...
	public ResNonActivePackDetails getHostInterface(IVRDataBean ivrDataBean, ReqNonActivePackDetails request) {
		...
		try{
			...
			logger.info(uniqueId + "|" + "TransNonActivePackDetails getHostInterface method start");
			wsdlProcessor();
			logger.info(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.info(uniqueId + "|" + "TransNonActivePackDetails getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			logger.info(uniqueId + "|" + "Before calling non active packs Service");
			...
			request = null;
			logger.info(uniqueId + "|" + "After calling non active packs Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
						((appProperties.getIVRProp(HostConstants.HOST_RESPONSE_ERROR_CODE_SUCCESS)).equalsIgnoreCase(errorCode)  || HostConstants.RESPONSE_CODE_FAILURE.equalsIgnoreCase(errorCode))){
					...
					logger.info(uniqueId + "|" + "Total Packs : " +nonActivePacksTokens.size());
					...
					{
						logger.info(uniqueId + "|" + "Pack Details : " +packDetails1.toString());
					}
