...
import com.servion.rcom.hostinterface.view.SRDetails;
...
public class TransServiceRequestDetails implements ITransaction{
	...
	public ResServiceRequestDetails getHostInterface(IVRDataBean ivrDataBean, ReqServiceRequestDetails request) {
		...
		try{
			...
			logger.info(uniqueId + "|" + "TransServiceRequestDetails getHostInterface method start");
			wsdlProcessor();
			System.out.println("end time" + Calendar.getInstance().getTime());
			logger.info(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.info(uniqueId + "|" + "TransServiceRequestDetails getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			logger.info(uniqueId + "|" + "Before calling service request Service");
			...
			request = null;
			logger.info(uniqueId + "|" + "After calling service request Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
						((appProperties.getIVRProp(HostConstants.HOST_RESPONSE_ERROR_CODE_SUCCESS)).equalsIgnoreCase(errorCode)) || "101".equalsIgnoreCase(errorCode)){
					...
					for(Token token : tokens)
					{
						...
						for(Param param : params)
						{
							if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_SR_ID))
								logger.info(uniqueId + "|" + "SR ID : " +param.getValue());
							
							if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_STATUS))
							{
								logger.info(uniqueId + "|" + "SR Status : " +param.getValue());
					...
					logger.info(uniqueId + "|" + "No of SR in open status : " +openSRDetailsTokenList.size());
						...
						for(Token openSRToken : openSRDetailsTokenList)
						{
								...
								for(Param param : listParam)
								{
									if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_SR_ID))
									{
										...
										logger.info(uniqueId + "|" + "SR ID : " +srDetails.getServiceRequestID());
									}
									if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_SRCREATION_DATE))
									{
										...
										logger.info(uniqueId + "|" + "SR Creation Date : " +srDetails.getServiceRequestCreationDate());
									}
									if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_SR_TYPE))
									{
										srDetails.setServiceRequestType(param.getValue());
										logger.info(uniqueId + "|" + "SR Type : " +param.getValue());
									}
									if(param.getId().equalsIgnoreCase(HostConstants.PARAM_SR_STATUS))
									{
						...
						Collections.sort(srDetailsList, new Comparator<SRDetails>() {
							...
							public int compare(SRDetails o1, SRDetails o2) {
								...
								Date date2 = null;
									...
									logger.info(uniqueId + "|" + "Parse Exception while parsing SR creation date for sorting." +e);
