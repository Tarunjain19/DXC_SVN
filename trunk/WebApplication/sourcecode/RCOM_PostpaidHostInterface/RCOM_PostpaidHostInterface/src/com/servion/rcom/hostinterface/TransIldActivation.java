...
public class TransIldActivation implements ITransaction{

	ReqIldActivation reqIldActivation = null;
	ResIldActivation resIldActivation = new ResIldActivation();
	...
	public ResIldActivation getHostInterface(IVRDataBean ivrDataBean , ReqIldActivation request) {
		...
		host.setHOSTID(HostTransactionID.TRANS_ILD_ACTIVATION);
		
		reqIldActivation = request;
		uniqueId = ivrDataBean.getSessionID();
		logger.debug(uniqueId + "|" + "TransIldActivation getHostInterface method start");
		wsdlProcessor();
		logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
		logger.debug(uniqueId + "|" + "TransIldActivation getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			serviceURL = appProperties.getIVRProp(HostConstants.CSS_ENDPOINT_URL_ILD_ACTIVATION);
			...
			request.setRequestType(appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_ILD_ACTIVATION));
			...
			Param ILDInfo = new Param();
			ILDInfo.setId(HostConstants.PARAM_ILD_ACTION_TYPE);
			ILDInfo.setValue(HostConstants.PARAM_ILD_ACTION_TYPE_ACTIVATION);
			ILDInfo.setType(HostConstants.PARAM_TYPE_STRING);
			
			request.getParam().add(ILDInfo);
			
			Param ILDInfo1 = new Param();
			ILDInfo1.setId(HostConstants.CUST_CATEGORY);
			ILDInfo1.setValue(HostConstants.HNI);
			ILDInfo1.setType(HostConstants.PARAM_TYPE_STRING);
			
			request.getParam().add(ILDInfo1);
			
			logger.debug(uniqueId + "|" + "Timestamp : " +request.getTimeStamp());
			logger.debug(uniqueId + "|" + "Before calling ILD Activation Service");
			
			//For Reporting
			host.setHOSTMETHOD(HostConstants.CSS_HOST_METHOD + HostConstants.PIPE + appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_ILD_ACTIVATION));
			...
			request = null;
			logger.debug(uniqueId + "|" + "After calling ILD Activation Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					resIldActivation.setMessageText(messageText);
			...
			resIldActivation.setResponseCode(responseCode);
			
			//For Reporting
			host.setHOSTOUTPARAMS(errorCode+HostConstants.PIPE+resIldActivation.getMessageText());
