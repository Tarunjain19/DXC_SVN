...
import com.servion.rcom.ppp.hostinterface.view.PlanDetails;
...
public class TransActivePlanDetails implements ITransaction{
	...
	public ResActivePlanDetails getHostInterface(IVRDataBean ivrDataBean , ReqActivePlanDetails request) {
		...
		try{
			...
			logger.debug(uniqueId + "|" + "TransActivePlanDetails getHostInterface method start");
			wsdlProcessor();
			logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
			logger.debug(uniqueId + "|" + "TransActivePlanDetails getHostInterface method end");
		...
		try {
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					logger.debug(uniqueId + "|" + "Total Plans : " +planTokens.size());
					...
					{
						logger.debug(uniqueId + "|" + "Plan Details : " +plnDetails1.toString());
					}
