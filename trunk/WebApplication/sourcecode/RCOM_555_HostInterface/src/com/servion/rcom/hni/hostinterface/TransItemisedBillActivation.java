...
public class TransItemisedBillActivation implements ITransaction{

	ReqItemisedBillActivation reqItemisedBillActivation = null;
	ResItemizedBillActivation resItemizedBillActivation = new ResItemizedBillActivation();
	...
	public ResItemizedBillActivation getHostInterface(IVRDataBean ivrDataBean , ReqItemisedBillActivation request) {
		...
		host.setHOSTID(HostTransactionID.TRANS_ITEMISED_BILL_ACTIVATION);
		
		reqItemisedBillActivation = request;
		uniqueId = ivrDataBean.getSessionID();
		logger.debug(uniqueId + "|" + "TransItemisedBillActivation getHostInterface method start");
		wsdlProcessor();
		logger.debug(uniqueId + "|" + "Response Code to Call Flow : " + responseCode);
		logger.debug(uniqueId + "|" + "TransItemisedBillActivation getHostInterface method end");
	...
	public void wsdlProcessor() {
		...
		try {
			...
			serviceURL = appProperties.getIVRProp(HostConstants.CSS_ENDPOINT_URL_IBA);
			...
			MDN = reqItemisedBillActivation.getMDN();
			...
			request.setRequestType(appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_ITEMISED_BILL_ACTIVATION));
			...
			Param ITBillInfo = new Param();
			ITBillInfo.setId(HostConstants.PARAM_ITBA_ACTION_TYPE);
			ITBillInfo.setValue(HostConstants.PARAM_ITBA_ACTION_TYPE_ACTIVATION);
			ITBillInfo.setType(HostConstants.PARAM_TYPE_STRING);
			
			request.getParam().add(ITBillInfo);
			
			logger.debug(uniqueId + "|" + "Timestamp : " +request.getTimeStamp());
			logger.debug(uniqueId + "|" + "Before calling Itemised Bill Activation Service");
			
			//For Reporting
			host.setHOSTMETHOD(HostConstants.CSS_HOST_METHOD + HostConstants.PIPE + appProperties.getIVRProp(HostConstants.CSS_REQUEST_TYPE_ITEMISED_BILL_ACTIVATION));
			...
			request = null;
			logger.debug(uniqueId + "|" + "After calling Itemised Bill Activation Service");
			...
			if(response != null)
			{
				...
				if((errorCode != null) && (!HostConstants.EMPTY.equalsIgnoreCase(errorCode)) && 
					...
					resItemizedBillActivation.setMessageText(messageText);
			...
			resItemizedBillActivation.setResponseCode(responseCode);
			
			//For Reporting
			host.setHOSTOUTPARAMS(errorCode+HostConstants.PIPE+resItemizedBillActivation.getMessageText());
