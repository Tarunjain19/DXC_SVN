...
import com.servion.rcom.hostinterface.view.TransactionDetails;
...
public class TransLastThreeTransactions implements ITransaction{
	...
	private String errorCode = null,txnAmt =null;
	...
	public void wsdlProcessor() {
		...
		try {
			...
			String serviceURL = appProperties.getIVRProp(HostConstants.CSS_ENDPOINT_URL);
					...
					if(lastThreeTransactionTokens.size() != 0)
					{
						for(Token transactionToken : lastThreeTransactionTokens)
						{
							...
							for(Param param : params)
							{
								if(param.getId().equalsIgnoreCase(HostConstants.PARAM_TXN_AMOUNT))
								{
									txnAmt = param.getValue();
									txnAmt = Utilities.adjustDecimal(logger, txnAmt);
									transactionDetails.setAmount(txnAmt);
								}
