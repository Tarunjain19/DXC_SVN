...
import com.rcom.bb.model.CreditLimitInfo;
 ...
 * Last generated by Orchestration Designer at: 2015-MAY-24  04:25:34 PM
 */
public class WS_CreditLimitDetails extends com.avaya.sce.runtime.Data {
	...
	public void requestBegin(SCESession mySession) {
		...
		try
		{
			...
			creditLimitInfo.setBAN(BAN);
			
			
			creditLimitInfo = IVRDBServiceInstance.getInstance().getCreditLimitInfo(ivrDataBean, creditLimitInfo);
			
			if(creditLimitInfo.getCreditLimit() != null && !Constants.EMPTY.equalsIgnoreCase(creditLimitInfo.getCreditLimit()))
			{
				
				DB_Result = Constants.SUCCESS;
				creditLimit = creditLimitInfo.getCreditLimit();
				trace.writeln(ITraceInfo.TRACE_LEVEL_DEBUG, "Credit Limit : " +creditLimit);
				
				key = appName+Constants.HYPEN_SEPERATOR+Constants.DAKC_PA_0013;
				initialPrompt = ivrProperties.getIVRProp(key);
				trace.writeln(ITraceInfo.TRACE_LEVEL_DEBUG, "Dynamic Prompt Key : "+key);
				dynamicList.add(creditLimit+Constants.currency);
				
				/** Invoking the playPromt method **/
				initialPrompt = DynamicMenu.playPrompt(dynamicList, initialPrompt, Constants.NA, Constants.DAKC_PA_0013, trace);
	...
	public boolean executeDataActions(com.avaya.sce.runtimecommon.SCESession mySession) throws Exception {
		...
		if(((com.avaya.sce.runtime.Condition)new com.avaya.sce.runtime.Condition("condition1", "appVariables:DB_Result", com.avaya.sce.runtime.Expression.STRING_EQUAL_IGNORE, "constants:success", true).setDebugId(5405)).evaluate(mySession)) {
			actions = new java.util.ArrayList(2);
			actions.add(new com.avaya.sce.runtime.varoperations.Assign("appVariables:transferCode", "TransferCodes:DAKC_TR_0016", false).setDebugId(8781));
			actions.add(new com.avaya.sce.runtime.Next("CreditLimit_PP", "(Success)").setDebugId(5408));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;
			...
			actions = new java.util.ArrayList(2);
			actions.add(new com.avaya.sce.runtime.varoperations.Assign("appVariables:transferCode", "TransferCodes:DAKC_TR_0057", false).setDebugId(5407));
			actions.add(new com.avaya.sce.runtime.Next("Linkdown_PP", "(Failure)").setDebugId(2909));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;
