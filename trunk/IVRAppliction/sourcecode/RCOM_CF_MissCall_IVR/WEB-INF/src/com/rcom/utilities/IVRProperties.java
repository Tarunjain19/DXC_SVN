...
public class IVRProperties{
	...
	private static IVRProperties uidaiProp = null;
	private static String configFileName = null;
	private static Properties storeProps = null;
	
	private IVRProperties(SCESession session) {
		refresh(session);
	}
	
	public static IVRProperties getInstance(String fName,SCESession session) {
	...
	public static IVRProperties getInstance(SCESession session) {
	
		if (uidaiProp == null){
			uidaiProp= new IVRProperties(session);
			if (session!=null){
	...
	private void refresh(SCESession session) {
		...
		try {
			...
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_DEBUG, " config file :"+configFile,session);
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_DEBUG, " config file :"+configFileName,session);
