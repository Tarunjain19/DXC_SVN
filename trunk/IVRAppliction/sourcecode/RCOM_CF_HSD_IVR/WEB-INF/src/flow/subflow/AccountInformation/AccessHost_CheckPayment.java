...
public class AccessHost_CheckPayment extends com.avaya.sce.runtime.Data {
	...
	public void requestBegin(SCESession mySession) {
		...
		try {
			...
			reqLastThreeTransactions.setMDN(mySession.getVariableField(IProjectVariables.GET_DATA_CARD, IProjectVariables.GET_DATA_CARD_FIELD_VALUE).getStringValue());
			...
			if((resLastThreeTransactions.getResponseCode()).equals(Constants.zero)){
				
				arrayTransactionDetails = resLastThreeTransactions.getTransactionDetailsList();
				mySession.getVariableField(IProjectVariables.POSTPAID__CHECK_PAYMENT, IProjectVariables.POSTPAID__CHECK_PAYMENT_FIELD_AMOUNT).setValue(arrayTransactionDetails.get(0).getAmount());
				mySession.getVariableField(IProjectVariables.POSTPAID__CHECK_PAYMENT, IProjectVariables.POSTPAID__CHECK_PAYMENT_FIELD_PAYMENT_TYPE).setValue(arrayTransactionDetails.get(0).getPmtType());
				...
				for (int i = 0; i < resLastThreeTransactions.getTransactionDetailsList().size(); i++) {
					if(resLastThreeTransactions.getTransactionDetailsList().get(i).getAmount()!=null){
						arrSMSData = new ArrayList<String>();
						...
						arrSMSData.add(resLastThreeTransactions.getTransactionDetailsList().get(i).getPmtType());
				...
				info.writeln(ITraceInfo.TRACE_LEVEL_DEBUG, " payment date + 6 days:"+paymentDate);
				info.writeln(ITraceInfo.TRACE_LEVEL_DEBUG, " Call date and payment date comparions:"+callDate.compareTo(paymentDate));
				
				if(callDate.compareTo(paymentDate)<=0){
					strFlag = Constants.Yes;
					/*strSMSFrame = appProperties.getIVRProp(Constants.CHECK_PAYMENT_Success);
					smsHashTable = setPushSMS.generatePushSMSDataMap(strSMSFrame, arrSMSData);*/
					
					mySession.getVariableField(IProjectVariables.POSTPAID__CHECK_PAYMENT, IProjectVariables.POSTPAID__CHECK_PAYMENT_FIELD_IS_DATE_VALID).setValue(Constants.Yes);
				}else{
					strFlag = Constants.No;
					
					/*strSMSFrame = appProperties.getIVRProp(Constants.check);
					smsHashTable = setPushSMS.generatePushSMSDataMap(strSMSFrame, arrSMSData);*/
					
					mySession.getVariableField(IProjectVariables.POSTPAID__CHECK_PAYMENT, IProjectVariables.POSTPAID__CHECK_PAYMENT_FIELD_IS_DATE_VALID).setValue(Constants.No);
				}
				...
				mySession.getVariableField(IProjectVariables.POSTPAID__CHECK_PAYMENT, IProjectVariables.POSTPAID__CHECK_PAYMENT_FIELD_PAYMENT_RECEIVED_DATE).setValue(strPaymentDate);
	...
	public boolean executeDataActions(com.avaya.sce.runtimecommon.SCESession mySession) throws Exception {
		...
		actions.add(new com.avaya.sce.runtime.varoperations.Assign("Postpaid_CallData:globalMenuType", "Postpaid_Constants:FeatureLevel", false).setDebugId(4488));
		actions.add(new com.avaya.sce.runtime.varoperations.Assign("Postpaid_CallData:repeatTries", "0", true).setDebugId(5308));
		if(evaluateActions(actions, mySession)) {
			return true;
		}
		actions = null;

		if(((com.avaya.sce.runtime.Condition)new com.avaya.sce.runtime.Condition("condition1", "Postpaid_Flag:isLinkDown", com.avaya.sce.runtime.Expression.STRING_EQUAL_IGNORE, "Postpaid_Constants:yes", true).setDebugId(3350)).evaluate(mySession)) {
			actions = new java.util.ArrayList(1);
			actions.add(new com.avaya.sce.runtime.Next("AccountInformation-ChkIntelligentLayer_CheckPayment", "LinkDown").setDebugId(3351));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;
			...
			actions = new java.util.ArrayList(1);
			actions.add(new com.avaya.sce.runtime.Next("AccountInformation-CheckEventBasedIL_CheckPayment", "Continue").setDebugId(3353));
			if(evaluateActions(actions, mySession)) {
				return true;
			}
			actions = null;
