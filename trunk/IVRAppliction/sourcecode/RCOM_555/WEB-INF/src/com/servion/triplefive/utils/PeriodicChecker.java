...
public class PeriodicChecker extends Thread{
	...
	String prepaidApplicationName = Constants.EMPTY_STRING;
	String postpaidApplicationName = Constants.EMPTY_STRING;
	...
	String sleepTime = Constants.EMPTY_STRING;
	 @Override
	    public void run()
	    {
		 	...
		 	prepaidApplicationName = AppProperties.GetInstance(Constants.IVR_CONFIG).getIVRProp(Constants.PREPAID_APPLICATION_NAME);
		 	postpaidApplicationName = AppProperties.GetInstance(Constants.IVR_CONFIG).getIVRProp(Constants.POSTPAID_APPLICATION_NAME);
			...
			/*** Thread Sleep  ***/
	    	   ...
	    	   /*** Service Updation***/ 
	    	   try{
	    		   objAppServiceUpdateInfo=objDBLoadOnStartUp.CheckAdminStatus(appNames + Constants.COMMA + prepaidApplicationName + Constants.COMMA + postpaidApplicationName);
	    	   }catch(Exception e){
	    	   ...
	    	   if(objAppServiceUpdateInfo!=null){
	    		  ...
	    		  for(String localAppName: keys){
		   		   ...
		   		   try{
		   			objServiceUpdateInfo = objProductServiceUpdateInfo.get(Constants.RECHARGEDETAIL);
		   			if(objServiceUpdateInfo!=null){
			   			if(Constants.RECHARGEDETAIL.equalsIgnoreCase(objServiceUpdateInfo.getServiceName())&&
			   					Constants.Y.equalsIgnoreCase(objServiceUpdateInfo.getStatus())){
			   				...
			   				objDBLoadOnStartUp.UpdateAdminStatus(localAppName, Constants.RECHARGEDETAIL);
			   			}else{
		   			...
		   			/***Check Special Tariff Offer ***/
		   		   try{
		   			objServiceUpdateInfo = objProductServiceUpdateInfo.get(Constants.STV);
		   			if(objServiceUpdateInfo!=null){
			   			if(Constants.STV.equalsIgnoreCase(objServiceUpdateInfo.getServiceName())&&
			   					Constants.Y.equalsIgnoreCase(objServiceUpdateInfo.getStatus())){
			   				logger.debug("STV DETAIL UPDATED");
			   				objDBLoadOnStartUp.loadSTVDetails(localAppName);
			   				logger.debug("UPDATED STV DETAIL Flag");
			   				objDBLoadOnStartUp.UpdateAdminStatus(localAppName, Constants.STV);
			   			}else{
